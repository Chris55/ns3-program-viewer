...
describe("/ns2/extern", () => {
    ...
    test("slotA.extern.octaveShift.value eq -7", async () => {
        const file = "slotA.extern.octaveShift.value eq -7.ns2p";
        const sut = await loadTestCase(root + file);
        sut.data.forEach((d) => {
            expect(d.actual).toEqual(d.expected);
        });
    ...
    test("slotA.extern.octaveShift.value eq 6", async () => {
        const file = "slotA.extern.octaveShift.value eq 6.ns2p";
        const sut = await loadTestCase(root + file);
        sut.data.forEach((d) => {
            expect(d.actual).toEqual(d.expected);
        });
